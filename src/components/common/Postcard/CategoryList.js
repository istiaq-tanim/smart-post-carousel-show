	...
	return (
		<div
			...
			style={{
				...
				"--taxonomy-border": taxonomyBorderStyle || "none",
				"--taxonomy-hover-border":
					taxonomyHoverBorderStyle === "none"
						? taxonomyBorderStyle
						: taxonomyHoverBorderStyle,
				"--taxonomy-border-size": `${
					taxonomyBorderWidthNormal?.[normalizedDeviceType] ?? 1
				}px`,
				"--taxonomy-hover-border-size": `${
					taxonomyBorderWidthHover?.[normalizedDeviceType] ?? 1
				}px`,
				...
				"--taxonomy-padding": `${
					taxonomyPadding?.[normalizedDeviceType]?.top ?? 0
				}px ${taxonomyPadding?.[normalizedDeviceType]?.right ?? 0}px ${
					taxonomyPadding?.[normalizedDeviceType]?.bottom ?? 0
				}px ${taxonomyPadding?.[normalizedDeviceType]?.left ?? 0}px`,

				"--taxonomy-margin": `${
					taxonomyMargin?.[normalizedDeviceType]?.top ?? 0
				}px ${taxonomyMargin?.[normalizedDeviceType]?.right ?? 0}px ${
					taxonomyMargin?.[normalizedDeviceType]?.bottom ?? 0
				}px ${taxonomyMargin?.[normalizedDeviceType]?.left ?? 0}px`,

				"--taxonomyListBetweenGap": `${
					taxonomyGap?.[normalizedDeviceType] ?? 4
				}px`,

				"--taxonomy-border-radius": `${
					taxonomyBorderRadiusNormal?.[normalizedDeviceType] ?? 0
				}px`,
				"--taxonomy-hover-border-radius": `${
					taxonomyBorderRadiusHover?.[normalizedDeviceType] ?? 0
				}px`,
			}}
		>
